<?php
/*
Plugin Name: WP Simple ParsPal Shopping cart
Version: v1.0.0
Author: Mobin Sadeghi Nasab
Author URI: http://www.mobinsoft.net/

Development and Translation:
Mostafa Amiri - http://www.samansystems.com/

Based on Work of:
Core Author: Ruhul Amin
Core URI: http://www.tipsandtricks-hq.com/
Description: Simple WordPress Shopping Cart Plugin, very easy to use and great for selling products and services from your blog, Integrated with ParsPal Merchant!
*/
...
add_option('wp_cart_empty_text', '   سبد خريد شما خالی است');
add_option('cart_return_from_parspal_url', get_bloginfo('wpurl'));
...
// Reset the Cart as this is a returned customer from parspal
...
function reset_wp_cart()
{
    ...
    header('Location: ' . get_option('cart_return_from_parspal_url'));
}
...
function print_wp_shopping_cart()
{
    ...
    $parspal_id = get_option('cart_parspal_id');
	$parspal_password = get_option('parspal_password');
	$toman_symbol = ' تومان';
    ...
    {   
	    ...
	    foreach ($_SESSION['simpleCart'] as $item)
	    {
       	...
       	if ($count)
       	{
			...
			$return = get_option('cart_return_from_parspal_url');
       
              	//$output .= "<form action=\"https://www.zarinpal.com/webservice/Simplepay\" method=\"post\" id=\"TransactionAddForm\">";
              	$output .= "<form action='http://merchant.parspal.com/postservice/' method='post' id='TransactionForm'/>";	
    			if ($count)
            		$output .= '<br /><center><input type="image" src="'.WP_CART_URL.'/images/buy.png"></center>';
       
    			$output .= $urls.'
			    <input type="hidden" id="MerchantID" value="'.$parspal_id.'" name="MerchantID"/>
				<input type="hidden" id="Password" value="'.$parspal_password.'" name="Password"/>
				<input type="hidden" id="Paymenter" value="'.'ميهمان'.'" name="Paymenter"/>
				<input type="hidden" id="Email" value="'.$admin_email.'" name="Email"/>
				<input type="hidden" id="Mobile" value="'.'09350001111'.'" name="Mobile"/>
				<input type="hidden" id="Price" value="'.($total+$postage_cost).'" name="Price"/>
				<input type="hidden" id="ResNumber" value="'.$desc.'" name="ResNumber"/>
				<input type="hidden" id="Description" value="'.$desc.'" name="Description"/>
				<input type="hidden" id="ReturnPath" value="'.$return.'" name="ReturnPath"/>
				</form>';          
       	}       
       	$output .= "       
       	</td></tr>
...
function show_wp_cart_options_page () {	
	$wp_simple_parspal_shopping_cart_version = "1.0.0";
    if (isset($_POST['info_update']))
    {
        ...
        update_option('cart_parspal_id', (string)$_POST["cart_parspal_id"]);
		update_option('parspal_password', (string)$_POST["parspal_password"]);
        ...
        update_option('cart_return_from_parspal_url', (string)$_POST["cart_return_from_parspal_url"]);
    ...
    $parspal_id = get_option('cart_parspal_id');
    $parspal_password = get_option('parspal_password');
    $return_url =  get_option('cart_return_from_parspal_url');
	...
	?>
 	<h2>ماژول آسان پرداز و سبدخريد پارس پال نسخه <?php echo $wp_simple_parspal_shopping_cart_version; ?></h2>
 	
 	<p>اين ماژول به شما امکان دريافت وجه آنلاين مربوط به سفارش را از طريق درگاه پارس پال و به کمک سرويس "آسان پرداز" پارس پال ميدهد. توجه کنيد که با توجه به اينکه اين سرويس از آسان پرداز پارس پال استفاده ميکند مشتری بايد پس از تکميل عمليات پرداخت، حتما شماره تراکنش را به شما اطلاع دهد و يا از طريق بخش تماس با ما شماره تراکنش را برای مديريت سايت ارسال کند. پس توجه داشته باشيد که حتما از مشتری خود درخواست نمائيد تا شماره تراکنش را در پايان تراکنش ياداشت نموده و به شما اطلاع دهد. استفاده از آسان پرداز پارس پال معايب و مزايايي دارد که از جمله مزايای آن ميتوان به عدم وابستگی ماژول به هاست و آی پی سرور و همچنين عدم نياز به درخواست درگاه پرداخت از پارس پال ميباشد. بدين صورت که کاربر برای استفاده از اين ماژول، نيازی به درخواست درگاه پرداخت از پارس پال نخواهد داشت.<br />
	</p>
    
     <fieldset class="options">
    <legend>نحوه استفاده:</legend>

    <p>1. برای افزودن لينک افزودن به سبد خريد از تگ <strong>[wp_cart:PRODUCT-NAME:price:PRODUCT-PRICE:end]</strong> در صفحات و يا نوشته های خود استفاده نماييد. PRODUCT-NAME با نام محصول و PRODUCT-PRICE با قيمت آن پر نماييد. به عنوان مثال: [wp_cart:ساعت مچی:price:15000:end] <br />
    همچنين براي انتخاب جزئيات محصول مي‌توانيد از عبارت روبه‌رو استفاده نماييد: <strong>[wp_cart:PRODUCT-NAME:price:15:shipping:2:var1[Size|Small|Medium|Large]:var2[Color|Red|Green]:end]</strong> </p>
...
echo '
	<div class="postbox">
	<h3><label for="title">تنظيمات پارس پال و سبد خريد</label></h3>
	<div class="inside">';

echo '
<table class="form-table">
<tr valign="top">
<th scope="row">شناسه درگاه (Merchant ID)</th>
<td><input type="text" name="cart_parspal_id" value="'.$parspal_id.'" size="40" /></td>
</tr>
<tr valign="top">
<th scope="row">کلمه عبور درگاه</th>
<td><input type="text" name="parspal_password" value="'.$parspal_password.'" size="40" /></td>
</tr>
<tr valign="top">
...
<td><input type="text" name="cart_base_shipping_cost" value="'.$baseShipping.'" size="5" /> <br />0 به معنای عدم هزينه برای ارسال است. </td>
</tr>
...
<td><input type="text" name="cart_return_from_parspal_url" value="'.$return_url.'" size="100" /><br />پس از طی مراحل خريد کاربر به اين صفحه باز می گردد</td>
</tr>
...
<td><input type="checkbox" name="wp_use_aff_platform" value="1" '.$wp_use_aff_platform.' /><br />برای اطلاعات بيشتر به  <a href="http://tipsandtricks-hq.com/?p=1474" target="_blank">WP Affiliate Platform plugin</a> رجوع کنيد.</td>
 ...
 ';
    echo 'از اين ماژول خوشت اوومده ؟! <a href="http://wordpress.org/extend/plugins/parspal-simple-shopping-cart/ target="_blank">يه نمره مشتی بهش بده پس!</a>'; 
}

function wp_cart_options()
{
     echo '<div class="wrap"><h2>تنظيمات ماژول آسان پرداز پارس پال</h2>';
...
{
     add_options_page('سبد خريد آسان پرداز پارس پال', 'آسان پرداز پارس پال', 'manage_options', __FILE__, 'wp_cart_options');  
}

function show_wp_parspal_shopping_cart_widget($args)
{
	extract($args);
...
function wp_parspal_shopping_cart_widget_control()
{
    ?>
    <p>
...
function widget_wp_parspal_shopping_cart_init()
{	
    $widget_options = array('classname' => 'widget_wp_parspal_shopping_cart', 'description' => __( "Display WP parspal Shopping Cart.") );
    wp_register_sidebar_widget('wp_parspal_shopping_cart_widgets', __('آسان پرداز پارس پال'), 'show_wp_parspal_shopping_cart_widget', $widget_options);
    wp_register_widget_control('wp_parspal_shopping_cart_widgets', __('آسان پرداز پارس پال'), 'wp_parspal_shopping_cart_widget_control' );
}
...
add_action('init', 'widget_wp_parspal_shopping_cart_init');
//add_filter('the_content', 'print_wp_cart_button',11);
add_filter('widget_text', 'print_wp_cart_button_new');
